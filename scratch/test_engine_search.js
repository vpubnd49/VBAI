...
async function main() {
  ...
  console.log(`Executing Engine Search for query: "${query}" against engine: vbai-legal-engine...`);
  
  const endpoint = `https://discoveryengine.googleapis.com/v1beta/projects/${projectId}/locations/global/collections/default_collection/engines/vbai-legal-engine/servingConfigs/default_search:search`;
  ...
  if (!response.ok) {
    ...
    console.error('Error executing engine search:', JSON.stringify(err, null, 2));
    process.exit(1);
  }
  ...
  console.log('==================================================');
  console.log(`Engine Search returned ${results.length} results:`);
  results.forEach((r, i) => {
