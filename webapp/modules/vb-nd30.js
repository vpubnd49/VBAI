...
const LOAI_VB = {
  ...
  quy_che:'QUY CH?', quy_dinh:'QUY ?NH', thong_bao:'THNG BO',
  huong_dan:'HU?NG D?N', chuong_trinh:'CHUONG TRNH', ke_hoach:'K? HO?CH',
  bao_cao:'BO CO', to_trinh:'T? TRNH', cong_van:'Cng van',
  giay_moi:'GI?Y M?I', hop_dong:'H?P ?NG', cong_dien:'CNG I?N',
  bien_ban:'BIN B?N',
};
...
export function renderVBND30(container) {
  const now = new Date();
  fs = { step:1, loai_van_ban:'thong_bao', co_quan_chu_quan:'', co_quan_ban_hanh:'', so_ky_hieu:'', dia_danh:'Lm ?ng', ngay:String(now.getDate()).padStart(2, '0'),thang:String(now.getMonth() + 1).padStart(2, '0'),nam:String(now.getFullYear()), trich_yeu:'', noi_dung:'', quyen_han_ky:'K tr?c ti?p', chuc_vu_ky:'', nguoi_ky:'', noi_nhan:'', kinh_gui:'', can_cu:'', dong_chuc_danh_1:'', dong_chuc_danh_2:'', dong_chuc_danh_3:'' };
  doRender(container);
}

function doRender(c) {
  c.innerHTML = `
    <div class="page-header"><div class="page-title">?? So?n VB Hnh Chnh (N30)</div><div class="page-subtitle">Chu?n Ngh? d?nh 30/2020/N-CP</div></div>
    <div class="steps-bar">${[1,2,3,4].map(i=>`<button class="step-indicator ${fs.step===i?'active':fs.step>i?'completed':''}" data-step="${i}"><span class="step-num">${fs.step>i?'?':i}</span><span>${['Lo?i VB','Thng tin','N?i dung','Xem & T?i'][i-1]}</span></button>`).join('')}</div>
...
function renderS2(sc,c) {
  sc.innerHTML=`
    <div class="panel-group">
      ...
      <div class="panel-body form-grid">
        ...
        <div class="form-group"><label class="form-label">CQ ban hnh <span class="required">*</span></label><input class="form-input" id="f2" value="${fs.co_quan_ban_hanh}" placeholder="VAN PHNG"></div>
      </div>
    </div>
    ...
    <div class="panel-group">
      ...
      <div class="panel-body form-grid">
        ...
        <div class="form-group"><label class="form-label">Ngy</label><input class="form-input" id="f5" value="${fs.ngay}" placeholder="17"></div>
        <div class="form-group"><label class="form-label">Thng / Nam</label><div style="display:flex;gap:8px"><input class="form-input" id="f6" value="${fs.thang}" placeholder="03" style="flex:1"><input class="form-input" id="f7" value="${fs.nam}" style="flex:1"></div></div>
      </div>
    </div>
    ...
    <div class="panel-group">
      ...
      <div class="panel-body form-grid full">
        <div class="form-group"><label class="form-label">Trch y?u <span class="required">*</span></label><input class="form-input" id="f8" value="${fs.trich_yeu}" placeholder="k?t lu?n c?a V? tru?ng t?i cu?c h?p giao ban"></div>
      </div>
    </div>
  ...
  sc.querySelector('#bn').onclick=()=>{sv();if(!fs.co_quan_ban_hanh){showToast('Nh?p CQ ban hnh','error');return}fs.step=3;doRender(c)};
}
...
function renderS3(sc,c) {
  ...
  sc.innerHTML=`
    <div class="panel-group">
      <div class="panel-header">
        <div class="panel-header-icon">??</div>
        N?i dung & Thng tin ph?
      </div>
      <div class="panel-body form-grid full">
        ${isKG?`<div class="form-group"><label class="form-label">Knh g?i</label><input class="form-input" id="fkg" value="${fs.kinh_gui}" placeholder="Cc don v?/c nhn nh?n (cch nhau d?u ;)"></div>`:''}
    ...
    <div class="panel-group">
      ...
      <div class="panel-body form-grid">
        ...
        <div class="span-2" style="margin-top: 16px; margin-bottom: 8px; font-weight: 700; font-size: 0.8rem; color: var(--daquy-500); text-transform: uppercase;">Dng ch?c danh (T?i da 3 dng)</div>
        <div class="form-group span-2"><label class="form-label">Dng 1</label><input class="form-input" id="fdcd1" value="${fs.dong_chuc_danh_1}" placeholder="VD: TM. ?Y BAN NHN DN"></div>
        <div class="form-group span-2"><label class="form-label">Dng 2</label><input class="form-input" id="fdcd2" value="${fs.dong_chuc_danh_2}" placeholder="VD: KT. CH? T?CH"></div>
        <div class="form-group span-2"><label class="form-label">Dng 3</label><input class="form-input" id="fdcd3" value="${fs.dong_chuc_danh_3}" placeholder="VD: PH CH? T?CH"></div>
      </div>
    </div>
  ...
  sc.innerHTML=`<div class="section-title">??? Xem tru?c & T?i file</div>
    <div class="preview-container">
      <table class="preview-header-table"><tr>
        ...
        <td style="width:60%;text-align:center"><div style="font-size:13pt;font-weight:bold">C?NG HA X H?I CH? NGHIA VI?T NAM</div><div style="font-size:14pt;font-weight:bold">?c l?p - T? do - H?nh phc</div><div style="border-top:1px solid #000;width:50%;margin:4px auto"></div><div style="font-style:italic">${fs.dia_danh}, ngy ${fs.ngay||'...'} thng ${fs.thang||'...'} nam ${fs.nam}</div></td>
      </tr></table>
      ...
      ${fs.kinh_gui?`<div class="preview-center" style="margin-top:8pt;margin-bottom:8pt">Knh g?i: ${fs.kinh_gui}</div>`:''}
...
async function genND30() {
  try {
    ...
    // Kinh gui
    if(fs.kinh_gui) {
      ch.push(new Paragraph({alignment:AlignmentType.CENTER,spacing:{before:240,after:240},children:[new TextRun({text:'Knh g?i: '+fs.kinh_gui,font:L.FONT,size:28})]}));
    }
    // Body
    ...
    showToast('?  t?i file DOCX thnh cng!');
    ...
    try {
      ...
      addDoc(collection(db, 'search_logs'), {
        query: `[T?o VB Hnh Chnh N30] ${LOAI_VB[fs.loai_van_ban]} - ${fs.trich_yeu}`,
