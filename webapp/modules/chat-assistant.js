...
const DEFAULT_MODEL = '';
const STRICT_MEETING_AUDIO_MODEL = '';
...
6. **VI. BẢNG DANH MỤC TRÍCH DẪN VĂN BẢN PHÁP LÝ CHÍNH THỨC & TẢI FILE**
   Trình bày bảng Markdown chuẩn. QUY TẮC SỐ LƯỢNG: TỐI ĐA 3-4 hàng trong bảng.
   ⚠️ CHỈ liệt kê:
   - Văn bản CHÍNH người dùng hỏi (ví dụ: Luật Đất đai 31/2024/QH15)
   - Tối đa 2-3 nghị định/thông tư hướng dẫn thi hành TRỰC TIẾP (nếu có)
   ⚠️ TUYỆT ĐỐI KHÔNG liệt kê:
   - Các luật LĨNH VỰC KHÁC dù được đề cập trong mệnh đề "sửa đổi bổ sung một số điều của Luật X, Luật Y" (ví dụ: không đưa Luật Quy hoạch, Luật Lâm nghiệp, Luật Thuỷ sản, Luật Thuế TNDN vào bảng VI khi người dùng hỏi về Luật Đất đai)
...
const CHAT_SESSION_ID = (() => {
  try {
    let value = sessionStorage.getItem(CHAT_SESSION_STORAGE_KEY);
    if (!value) {
      value = `chat_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`;
      sessionStorage.setItem(CHAT_SESSION_STORAGE_KEY, value);
    }
    return value;
  } catch {
    return null;
...
export function initChat(apiKey, modelName = '') {
  const normalizedModel = normalizeModelName(modelName);
  currentModelName = normalizedModel;
  if (!currentModelName) {
    console.warn('Chat init deferred: AI model is required in system configuration.');
  }
