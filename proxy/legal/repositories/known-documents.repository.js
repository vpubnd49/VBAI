...
function mapBosungEntryToCanonical(entry, matchedDocNumber = null) {
  ...
  if (isRelationMatch) {
    ...
    return {
      ...
      title: entry.title || `Văn bản ${docNum}`,
      title_is_placeholder: false,
      document_type: entry.document_type || entry.loai_van_ban || null,
      topic_aliases: entry.topic_aliases || [],
      query_patterns: entry.query_patterns || [],
      issuer: entry.issuer || entry.co_quan_ban_hanh || null,
      issue_date: entry.issue_date || entry.ngay_ban_hanh || null,
      effective_date: entry.effective_date || entry.ngay_hieu_luc || null,
      effective_status: entry.effective_status || entry.tinh_trang_hieu_luc || 'unknown',
      status_as_of: entry.status_as_of || null,
      replaces: [],
      amends: [],
